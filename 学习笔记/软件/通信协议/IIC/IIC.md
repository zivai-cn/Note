...
# 协议操作
- 起始位：要发送数据的设备拉低电平，总线其余所有设备检测到下降沿，获知要发送数据。
- 发送、接收信息：SCL低电平，A在总线上放置信息；SCL高电平，B从总线上读取信息。
- ACK应答位：ACK 发生在第 9 个 SCL 时钟周期（前 8 个周期传数据），是由接收方把 SDA 拉低。发送方在第 9 个时钟周期释放 SDA 控制权，并检测 SDA 是否被拉低以确认 ACK。若 SDA 仍为高，则表示 NACK（不应答）。
- 终止位：终止条件是 SCL 为高电平时，SDA 由低变高（上升沿）。
# 数据帧格式
## 发送数据
