...
**复盘 10 问（20 分钟）**
1. 队列是"拷贝"数据还是"传递指针"？这对内存有什么影响？答：传递指针，内存中的数据只是被调取，数据储存地址并没有被修改。答：拷贝，队列在创建时就会划分出一块区域，写入调出消息时，就是直接复制数据到这个队列区域中。
2. `portMAX_DELAY` 的值是多少？设为 0 又是什么含义？答：值可以任意设置，是唤醒的时间，设为0就是只有传递数据时才会被唤醒。答：有最大值100，最小值是0，设为0就是不等待，只要队列空或满直接返回失败，设置为portMAX_DELAY，就是无限等待，直到阻塞成功。
3. 如果多个任务同时等待同一个队列的数据，当数据到达时谁先被唤醒？答：不清楚。答：任务优先级越高，越先被唤醒。
4. 队列可以传递大于 `uxItemSize` 的数据吗？如果可以，怎么做？答：不能。答：一般意义上不可以，但是创建队列数据大小可以是结构体的指针，此时是可以看做不受限的。注意指针指向地址在传递给消费者之前不能是空的，最好使用静态分配。
5. 队列为空时调用 `xQueueReceive()` 和队列满时调用 `xQueueSend()`，各会发生什么？答：空时，会将数据塞入队列，满时，数据会被直接舍弃。答：阻塞等待直到有数据或者超时，阻塞等待直到有空位或者超时。
6. 为什么 `xQueueCreate` 的参数叫 `uxItemSize` 而不是 `uxItemType`？答：不清楚。答：队列的创建，只在意大小，不在意是什么类型的数据。
7. 能从队列中"偷看"数据而不移出吗？（提示：`xQueuePeek`）答：没见过提示的这个函数。答：可以，使用`xQueuePeek()`函数。
8. 队列创建后可以动态改变大小吗？如果不能，怎么办？答：可以动态改变大小。答：不能，如果不够，只能复制出数据到达一个更大的队列，再删除原来的队列。
9. `xQueueOverwrite` 和 `xQueueSend` 有什么区别？（提示：队列长度为 1 时）答：没用过覆写。答：overwrite会直接用新数据覆盖原来是数据，也就是塞入新数据，舍弃原来的首数据。
10. 队列操作返回 `pdPASS` 和 `pdFAIL`，但还有个 `errQUEUE_FULL` 是什么情况？答：err是队列爆满。答：pass的值是1，fail和full的值都是0。
...
1. 二值信号量和互斥锁的核心区别是什么？（提示：优先级继承）答：互斥锁就是在二进制信号量的基础上增加了优先级继承功能，可以提高低优先级任务的优先级，保证高优先任务不被中等优先级任务阻塞。
2. 为什么中断中只能用 `FromISR` 版本的 API？在中断中调用普通 API 会怎样？答：会直接卡死程序，因为中断中不允许阻塞。答：尝试获取一次值，获取不到就立刻返回，不会将任务置suspended。
3. `xSemaphoreGiveFromISR` 中的 `pxHigherPriorityTaskWoken` 参数是干什么的？为什么必须调用 `portYIELD_FROM_ISR`？答：是否执行切换任务的功能。需要对任务优先级进行判断确定是否执行切换优先级的任务。答：参数是是否有高优先级的任务需要执行。这个函数是在切换退出中断时执行的函数。
4. 互斥锁可以在 A 任务中 Take，在 B 任务中 Give 吗？为什么？答：不能。具体细节不清楚。答：互斥锁中记录了持有锁的句柄。
5. 递归互斥锁（Recursive Mutex）和普通互斥锁有什么区别？答：不知道。答：可以让一个任务多次获得锁，嵌套函数。
6. 计数信号量初始值设为 0 有什么特殊用法？答：不知道。答：默认上电阻塞take。
7. `xSemaphoreTake` 返回 `pdFALSE` 意味着什么？和 `pdPASS` 的关系？答：false说明获取失败，也就是信号量为空，pass则是获取成功。
8. 如果忘记调用 `xSemaphoreGive` 释放互斥锁会发生什么？低优先级的任务不能恢复到原来的优先级。答：同时还有死锁，其他等待锁的任务全部阻塞。
9. 信号量和队列在实现上有何关系？（提示：看源码会发现信号量就是特殊的队列）答：信号量就是一个单位的队列。
10. 什么是"死锁"？如何避免？答：优先级反转，使用互斥信号量。答：两个任务互相持有对方的锁，都能工作，解决方案多样。
