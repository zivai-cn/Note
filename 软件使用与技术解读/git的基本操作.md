...
## （二）调用仓库
1. 目的：在使用一些库函数时，如果官方进行了更新，我们就需要将官方的git仓库下载下来，再替换到我们的项目中，十分麻烦，实际上有更加便捷的操作。
2. 操作：
- 复制目标仓库的开源地址（SSH格式）
- 在git命令行（git bash）中执行操作：
```c
git clone 开源地址
```
- 当后续仓库更新后，在git bash中执行新命令：
```c
git pull
```
## （三）更改网络端口连接方式
在通过git将项目推送到仓库时，发现无法连接到github，经过检查发现是github的https协议端口被学校的校园网给墙掉了，于是修改了github仓库的连接端口为SSH协议。