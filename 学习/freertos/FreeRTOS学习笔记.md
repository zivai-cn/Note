...
```
3. 互斥信号量
- 问题：优先级反转——当多个优先级不同的任务都调用一个资源时，会出现下面的情况，（任务设计：获取信号量，运行，释放信号量）高优先级任务获取信号量，运行，释放信号量，接着信号被低优先级的执行。此时如果低优先级任务的运行时间>高优先级任务的运行时间，也就是说低优先级任务没有释放信号量，此时高优先级任务会尝试获取信号量，最终失败。也就是说，优先级低的任务反而阻塞了高优先级的任务。
- 解决方案：