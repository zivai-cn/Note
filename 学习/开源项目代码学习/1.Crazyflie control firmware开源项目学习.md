...
void workerInit()
{
  if (workerQueue)//防重复初始化，如果任务被创建过，直接返回
    return;  
  workerQueue = STATIC_MEM_QUEUE_CREATE(workerQueue);//创建静态队列
  DEBUG_QUEUE_MONITOR_REGISTER(workerQueue);//设置监控
  STATIC_MEM_TASK_CREATE(workerTask, workerTask, WORKER_TASK_NAME, NULL, WORKER_TASK_PRI);//创建freertos任务
}

void workerTask(void *arg)//核心执行功能的任务函数，在队列中塞入的任务在workerTask中执行
{
  struct worker_work work;  
  if (!workerQueue)//如果任务没有成功创建，会直接退出
    return;  
  while (1)
  {
    xQueueReceive(workerQueue, &work, portMAX_DELAY)//将workerQueue队列中任务转移到work
    if (work.function)//确认拿到的任务里面函数指针不为空
      work.function(work.arg);//执行功能
  }
}

int workerSchedule(void (*function)(void*), void *arg)//任务投递函数，生产者通过此函数将任务传递给workerTask
{
  struct worker_work work;
  if (!function)
    return ENOEXEC;  
...
```
工作流程：存在一个任务A，在任务A执行过程中，需要执行任务B，但是A需要持续工作，在A中调用workerSchedule，将B任务填入workerTask，workerTask检测到任务，阻塞将会被唤醒，执行B任务。
# 三、附属小知识
### C 预处理器 ## 符号说明
