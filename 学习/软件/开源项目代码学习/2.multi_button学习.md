...
// Configuration
static int demo_mode = 1;//演示控制开关
static int verbose_mode = 0;//输出开关，如果打开，将会输出更多信息。
//信号处理回调函数，接收到SIGINT信号，将通过running关闭程序。
...
//通用事件处理函数。verbose_mode控制的函数，对于事件打印信息。
...
//具体时间回调函数。是对通用事件处理函数的一次再封装，主要是为了给generic_event_handler函数调用有一个直观的命名。
...
//配置按钮特殊处理函数。config_state作为状态计数器，config_state的值被修改后实现保留，在本函数中实现每一次点击轮次执行不同的功能。
...
//按钮初始化函数。button_init添加按键信息，enable_all_events判断是否启用全部事件，然后按情况调用button_attach，给按键的事件绑定回调函数。最后启动按键状态机。
...
//全部按钮初始化函数。是对init_button的一次打包封装，对多个按键一同初始化。
...
    }
    usleep(500000);
    //**配置按钮测试**。按钮 3 点击 3 次，每次切换 `config_state`，依次执行：切换详细模式 → 切换演示模式 → 重置所有按钮。
...
    printf("==========================================\n");
    //首位是文件名称，不需要读取。从第一位开始读取。判断是否开启详细输出，以及是否是演示模式。
...
    }
    //只要在running，就持续通过button_ticks进行按键扫描。
    while (running) {
        button_ticks();
